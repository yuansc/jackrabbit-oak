 ...
 */
public class JGitNodeBuilder extends MemoryNodeBuilder {
    private static final Logger LOG = LoggerFactory.getLogger(JGitNodeBuilder.class);
    ...
    @Nonnull
    private final JGitWriter writer;
    ...
    private JGitNodeBuilder(
            @Nonnull JGitNodeBuilder parent,
            @Nonnull String name,
            @Nullable BlobStore blobStore,
            @Nonnull JGitReader reader,
            @Nonnull JGitWriter writer) {
    ...
    @Nonnull
    @Override
    public JGitNodeState getNodeState() {
        try {
            NodeState state = super.getNodeState();
            JGitNodeState sState = new JGitNodeState(reader, writer, blobStore, writer.writeNode(state));
            if (state != sState) {
                set(sState);
                if(!isChildBuilder()) {
                    updateCount = 0;
    ...
    private void set(JGitNodeState sState) {
    ...
    protected MemoryNodeBuilder createChildBuilder(String name) {
        return new JGitNodeBuilder(this, name, blobStore, reader, writer);
    }

    @Override
    public Blob createBlob(InputStream stream) throws IOException {
        return new JGitBlob(blobStore, writer.writeStream(stream));
    }
